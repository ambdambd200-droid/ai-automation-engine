...
# Default Chromium executable (installed by playwright install)
CHROMIUM_PATH = os.environ.get(
    "SESSION_CHROMIUM_PATH",
    r"C:\Users\A\AppData\Local\ms-playwright\chromium-1223\chrome-win\chrome.exe",
)

# Whether to open a visible browser window (False for headless)
HEADLESS = os.environ.get("SESSION_HEADLESS", "false").lower() in ("true", "1", "on")


class SessionManager:
    """
    Wraps Playwright with storage_state persistence.
    ...
    def __init__(self, platform: str, signup_url: str, home_url: str,
                 ...
                 wait_for_seconds_after_signup: int = 8):
    ...
    def __enter__(self):
        ...
        if not self.headless and Path(CHROMIUM_PATH).exists():
            launch_kwargs["executable_path"] = CHROMIUM_PATH
        elif not self.headless:
            # Try default browser; if missing, fall back to headless
        ...
        try:
            input(">>> Press ENTER after you complete signup/login: ")
            print("[session_manager] Saving session immediately on user input.")
        except EOFError:
            # No interactive stdin; just sleep the configured time
            print(f"[session_manager] No interactive stdin; sleeping {self.wait_after_signup}s...")
            time.sleep(self.wait_after_signup * 60)
